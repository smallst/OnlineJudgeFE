<template>
  <div class="view">
    ...
    <el-dialog title="Download Collection Submissions"
               width="30%"
...
<script>
  ...
  export default {
    ...
    filters: {
      contestStatus (value) {
        return COLLECTION_STATUS_REVERSE[value].name
      }
    },
    methods: {
      // 切换页码回调
      ...
      openDownloadOptions (collectionId) {
        this.downloadDialogVisible = true
        this.currentId = collectionId
      },
      downloadSubmissions () {
        let excludeAdmin = this.excludeAdmin ? '1' : '0'
        let url = `/admin/download_submissions?collection_id=${this.collectionId}&exclude_admin=${excludeAdmin}`
        utils.downloadFile(url)
      },
      goEdit (collectionId) {
        this.$router.push({name: 'edit-' + this.type, params: {collectionId}})
      },
