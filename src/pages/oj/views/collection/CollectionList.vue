  ...
  export default {
        ...
        collectionTableColumns: [
          {
            ...
            render: (h, params) => {
              return h('Button', {
                ...
                on: {
                  click: () => {
                    this.$router.push({name: 'collection-details', params: {collectionID: params.row._id}})
                  }
                },
                style: {
                  padding: '2px 0'
    ...
    methods: {
      ...
      getCollectionList () {
        ...
        api.getCollectionList(this.type, offset, this.limit, this.query).then(res => {
          this.loadings.table = false
          this.total = res.data.data.total
          this.collectionList = res.data.data.results
          if (this.isAuthenticated) {
            this.addStatusColumn(this.collectionTableColumns, res.data.data.results)
          }
        }, res => {
          this.loadings.table = false
        })
      ...
      handleTagsVisible (value) {
        if (value) {
          this.collectionTableColumns.push(
            {
          ...
          this.collectionTableColumns.splice(this.collectionTableColumns.length - 1, 1)
        }
      },
      onReset () {
      ...
      pickone () {
        api.pickone().then(res => {
          ...
          this.$router.push({name: 'collection-details', params: {collectionID: res.data.data}})
        })
      }
    },
    computed: {
